...
def remove_warining(pairs):
    ...
    warning_list = index.train_ignore_index
...
def divide_dataset(pairs):
    ...
    train_list = index.train_index #+ index.val_index
...
def train(model, tokenizer, train_data, valid_data, args, augment):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch > 0:   
            # 나중에서는 스텝마다 저장해서 평가하자 - 지금은 시간문제 때문에 epoch마다            
            ...
            nsml.report(step=epoch, scope=locals(), summary=True,
                        valid__loss_sent2=val_loss, valid__token_ppl2=math.exp(val_loss_token),
                        valid__em2=val_em, valid__gleu2=val_gleu)

            if val_gleu > best_val_gleu:
                best_val_gleu = val_gleu
                nsml.save("best")

        nsml.save(str(epoch) + "best")        
...
def main():
    ...
    if args.mode == "train":
        ...
        #train_data = remove_warining(train_data)
        ...
        #nsml.load(checkpoint='best', session='KR95444/airush2021-2-4/584')
        nsml.load(checkpoint='best', session='KR95444/airush2021-2-4/1408')
        #nsml.load(checkpoint='2', session='KR95444/airush2021-2-4/472')
        #nsml.load(checkpoint='best', session='KR95444/airush2021-2-4/249')
